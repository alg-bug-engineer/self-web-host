...
6. 本机运行 We-MP-RSS；公众号采集任务每小时第 7 分钟更新本机 RSS，网站同步任务在第 17 分钟从回环地址读取，只接收本人公众号最近 31 天已公开文章，经 Feed 身份、时间、篇数、正文长度与生产构建校验后创建 PR、等待 CI、自动合并并由现有生产流水线部署；管理后台始终不公开。
...
We-MP-RSS 每小时第 7 分钟采集公众号；`cn.ai-knowledgepoints.wechat-site-sync` 在每小时第 17 分钟和用户登录时读取 RSS。导入端固定校验 Feed 身份、最近 31 天、单次最多 12 篇、正文至少 200 字，并用微信原文链接去重；不批量导入旧档。发现新文章后，在隔离 Git worktree 中安装依赖并完成生产构建，再创建 PR、等待 CI、自动合并并等待公网健康接口返回目标提交号。当前工作区的未提交文件不会被读取、暂存或覆盖。

安装或重装本机网站同步任务：

```bash
npm run schedule:wechat-sync:install
```
