...
6. 本机运行 We-MP-RSS；公众号采集任务每小时第 7 分钟更新本机 RSS，Codex 自动化“公众号文章自动同步到网站”在第 17 分钟从回环地址读取，只接收本人公众号最近 31 天已公开文章，经 Feed 身份、时间、篇数、正文长度与生产构建校验后创建 PR、等待 CI、自动合并并由现有生产流水线部署；管理后台始终不公开。
...
19. We-MP-RSS 自身的定时采集负责控制微信请求频率，网站同步任务只读取已有 Feed，不额外调用微信列表接口。导入使用原文 URL 去重；长链接校验公众号 `biz`，We-MP-RSS 当前输出的 `/s/<token>` 短链接同时校验条目 ID 与固定 Feed ID `MP_WXS_3212677307`。原始 Feed、登录令牌、Cookie 和文章正文不会写入私有运营状态。
...
- 公众号 RSS 服务固定监听本机 `127.0.0.1:8001`，Feed ID 为 `MP_WXS_3212677307`。GitHub Actions 无法访问这个回环地址，因此由本机 launchd 读取并通过 GitHub PR 进入现有部署流水线。
...
- 数据：保存在本机 Docker 持久卷，应纳入本机数据备份。
- 管理端：仅 `127.0.0.1:8001`，禁止通过路由器端口映射、隧道或公网反向代理开放。
- 当前公众号：`芝士AI吃鱼`，Feed ID `MP_WXS_3212677307`。
- 微信授权 Token 有有效期；失效后打开本机管理页面重新扫码。

We-MP-RSS 每天 00:30、12:30 采集公众号；Codex 本机自动化“公众号文章自动同步到网站”在每天 00:45、12:45 读取 RSS，给采集任务预留 15 分钟。导入端固定校验 Feed 身份、最近 31 天、单次最多 12 篇、正文至少 200 字，并用微信原文链接去重；不批量导入旧档。文章中的微信图片只允许从可信图片域下载，带公众号 Referer 获取原图后保存到本站 `public/images/wechat/<文章 slug>/`，MDX 改用本站地址；任一微信图片下载或类型校验失败时整篇停止发布，避免上线防盗链占位图。页面同时使用 `no-referrer` 作为遗留外链兜底。发现新文章后，在隔离 Git worktree 中安装依赖并完成生产构建，再创建 PR、等待 CI、自动合并并等待公网健康接口返回目标提交号。当前工作区的未提交文件不会被读取、暂存或覆盖。

手动做一次只验证、不创建 PR 的同步：

```bash
WECHAT_SITE_SYNC_DRY_RUN=true npm run sync:wechat:local
```

微信可能返回 `200013` 频率控制。网站同步不会触发额外采集；频控期间 RSS 会保留已有内容，待 We-MP-RSS 后续成功采集后自动进入网站。

RSS 没有新条目时，网站同步以“新增 0 篇”正常结束，不创建空 PR，也不触发部署。RSS 无法访问、XML 无效、生产构建或 CI 失败时任务明确失败并保留日志。
