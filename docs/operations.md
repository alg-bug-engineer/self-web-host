...
We-MP-RSS 每小时第 7 分钟采集公众号；Codex 本机自动化“公众号文章自动同步到网站”在每小时第 17 分钟读取 RSS。导入端固定校验 Feed 身份、最近 31 天、单次最多 12 篇、正文至少 200 字，并用微信原文链接去重；不批量导入旧档。发现新文章后，在隔离 Git worktree 中安装依赖并完成生产构建，再创建 PR、等待 CI、自动合并并等待公网健康接口返回目标提交号。当前工作区的未提交文件不会被读取、暂存或覆盖。
